...
const CHART_COLORS = ["#00ffcc", "#00ccff", "#7c3aed", "#ec4899", "#f59e0b", "#10b981"];
...
function StatCard({ icon: Icon, label, value, sub, color }: {
  ...
  return (
    <div className="rounded-xl border border-border/40 bg-card/60 p-5 flex items-start gap-4">
      <div className={"flex h-10 w-10 items-center justify-center rounded-lg shrink-0 " + (color ?? "bg-primary/10 text-primary")}>
...
export default function Admin() {
  ...
  const pieData = stats
    ? [
        ...
        { name: "Errors", value: stats.totalErrors },
      ]
    : [];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      {/* Top bar */}
      <div className="sticky top-0 z-30 border-b border-border/40 bg-background/95 backdrop-blur px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-lg bg-primary/10 flex items-center justify-center">
            <Shield className="h-4 w-4 text-primary" />
          </div>
          <div>
            <h1 className="text-sm font-bold leading-none">Trustbit Admin</h1>
            <p className="text-xs text-muted-foreground">Control Panel</p>
          </div>
          <div className="ml-2 flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-green-500/10 border border-green-500/20">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={"text-xs px-3 py-1.5 rounded-lg border transition-colors " +
              (autoRefresh ? "bg-primary/10 border-primary/30 text-primary" : "border-border/40 text-muted-foreground hover:border-primary/30")}
          >
          ...
          <button
            onClick={fetchStats}
            disabled={loading}
            className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-border/40 hover:border-primary/30 transition-colors text-muted-foreground hover:text-foreground"
          >
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard icon={CheckCircle2} label="Success Rate" value={stats.successRate.toFixed(1) + "%"}
                sub={stats.totalErrors + " errors total"}
                color={stats.successRate >= 95 ? "bg-green-500/10 text-green-500" : "bg-yellow-500/10 text-yellow-500"} />
              <StatCard icon={Clock} label="Avg Response" value={stats.avgResponseMs + "ms"}
            ...
            <div className="rounded-xl border border-border/40 bg-card/40 p-5">
              ...
              {stats.topEndpoints.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-8">No requests recorded yet. Hit some endpoints and refresh.</p>
              ) : (
                <ResponsiveContainer width="100%" height={Math.max(200, stats.topEndpoints.length * 32)}>
                  <BarChart data={stats.topEndpoints} layout="vertical" margin={{ top: 0, right: 60, bottom: 0, left: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" horizontal={false} />
                    <XAxis type="number" tick={{ fontSize: 10, fill: "#666" }} tickLine={false} axisLine={false} />
                    <YAxis type="category" dataKey="path" width={180} tick={{ fontSize: 10, fill: "#888" }} tickLine={false} axisLine={false} />
                    <Tooltip
                      contentStyle={{ background: "#0a0a0a", border: "1px solid #333", borderRadius: 8, fontSize: 12 }}
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-border/10">
                    {stats.recentRequests.length === 0 ? (
                      <tr><td colSpan={6} className="text-center py-8 text-muted-foreground">No requests yet</td></tr>
                    ) : (
                      stats.recentRequests.map((r) => (
                        <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="px-4 py-2 font-mono text-foreground/80 max-w-xs truncate">{r.path}</td>
                          <td className="px-4 py-2">
                            <span className={"px-1.5 py-0.5 rounded font-mono font-bold text-xs " + statusBg(r.status)}>
                              {r.status}
                            </span>
                          </td>
                          <td className={"px-4 py-2 font-mono " + (r.ms > 2000 ? "text-yellow-400" : "text-muted-foreground")}>
                            {r.ms}ms
                          </td>
                          <td className="px-4 py-2 font-mono text-muted-foreground">{formatBytes(r.bytes)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
            ...
            {/* Top endpoints avg response time */}
            <div className="rounded-xl border border-border/40 bg-card/40 p-5">
              <div className="flex items-center gap-2 mb-4">
                <Clock className="h-4 w-4 text-primary" />
                <h2 className="font-semibold text-sm">Avg Response Time by Endpoint (ms)</h2>
              </div>
              {stats.topEndpoints.length === 0 ? (
